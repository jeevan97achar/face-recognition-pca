# PARALLELIZATION ATTEMPTS FOR Face_ReCog.py code
...
# Libraries used for parallelization
import multiprocessing as mp
from threading import Thread
import cupy as cp
import ray
from timebudget import timebudget
from numba import cuda, njit, jit, vectorize, float64, complex128
# Importing images 
...
# Finding the eigen vectors from the covariance matrix  - SERIAL VERSION
...
for i in range(len(images_df)):
    temp = (image_eigvecs[i] + images_df_mean).to_numpy()
...
zero_mat = np.zeros(shape=[len(images_df)])
